...
| **Relevance** | The threat model names strict CSP as the primary mitigation for the project's central acknowledged weakness (R-01): the relay serves the JavaScript that performs encryption, so a compromised server can ship key-stealing code. With browser-side encryption in place, an injected script reads plaintext *before* encryption, defeating invariant 1 without ever touching the server's stored ciphertext. CSP is what raises the cost of that attack. |
