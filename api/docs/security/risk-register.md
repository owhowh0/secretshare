...
| R-14 | Administrative interfaces are exposed: the Traefik dashboard on port 8080 of all host interfaces (`--api.insecure=true`), and the Keycloak admin console under `/keycloak/admin` through the public edge. Traefik has the Docker socket mounted. | T-20, T-27 | Socket mounted read-only | 2 | 4 | 8 | Medium |
...
| R-14 | Remove `--api.insecure=true` or bind the dashboard to `127.0.0.1`; restrict `/keycloak/admin` to an internal entrypoint or an address allowlist; replace the socket mount with a read-only Docker socket proxy. | Mitigate | DevOps | October | 1 × 3 = 3 |
